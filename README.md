...
- Certificate errors fail closed. WebKit evaluates every server certificate with its own trust policy (including fetching missing intermediates); if it rejects one, Graphene shows “<host>’s certificate isn’t trusted, so Graphene didn’t load the page.” with a Try again action and no “visit anyway” bypass. Certificate details in Site Controls are read from the trust WebKit provides, not merely HTTPS in the address.

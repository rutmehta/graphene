...
- Shell: persisted Sidebar and Top tabs layouts, expanding address/command results, light/dark/system appearance, space-tinted chrome, page gutters and optional page-font override. Shell geometry follows the Arc-look tokens in `docs/design/arc-look.md`: 36pt rows at 40pt pitch, 13pt labels and 16pt favicons; favorites in three columns (four at a 300pt content width) with 44pt tiles and 10pt gaps; an 8pt gap around the page card and a 32pt page toolbar. The sidebar has a 180–360pt resize divider and collapsed edge-peek implementation. Library exposes Threads, Vault, Mail and Board. The traffic band is draggable, with aligned window/navigation controls.
...
- Settings: native Settings window with a 180pt page list and grouped forms for General, Appearance, Spaces/Routing, Sites, Boosts, Profiles, Import, Privacy, Search, AI, Shortcuts, Mail and Advanced. Settings JSON import/export preserves existing tab/space collections. URL-glob routing uses the first enabled matching destination. Onboarding (a 540×420 sheet) selects layout, picks a space colour from three live swatches that render the real chrome gradient, and offers default-browser setup/import.
- Threads: navigation visits, originating-page edges, space-scoped list/detail/map, source text, resumable browsing, streamed summaries and Markdown export (Ask and Export sit in the view's 32pt top bar). Threads record browsing provenance rather than replacing tab folders. Threads, Vault, Mail and Board render inside the page card under a 32pt bar with the view title and its glyph actions.
- Vault: selected-text highlights and page notes, grid/detail/filtering, edit/delete, source provenance, individual Markdown files and Ask my notes (Ask, Open folder and Delete are top-bar glyphs). Save notifications offer View. Legacy daily Markdown exports are not rewritten by edits/deletes.
...
- Mail: retained read-only Gmail integration and web fallback when OAuth is not configured. The inbox uses system type throughout (no serif headings) with refresh and Open in Gmail in the top bar. WP9 fixes its local callback listener's completion/socket lifecycle; live Gmail authentication was not tested.

...
struct ImportSettings: View {
    ...
    var body: some View {
        Form {
            Section {
                SettingsHelp("Import bookmarks and history without changing the source browser. Passwords are never imported. Arc imports Spaces, pins and Favorites, not Today tabs or history.")
                ...
                SettingsHelp("Safari access may require System Settings → Privacy & Security → Full Disk Access → Graphene, then restart Graphene. Chrome defaults to its Default profile; choose a Bookmarks file for another profile. No permissions are granted automatically.")
            }
            if busy || batch != nil || !status.isEmpty {
                Section {
                    ...
                    if !status.isEmpty { SettingsHelp(status).textSelection(.enabled) }
                }
            }
        }.sheet(isPresented: $summary) {
            SettingsSheet(title: "Import summary", width: 480) {

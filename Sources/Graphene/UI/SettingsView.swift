...
struct SettingsView: View {
        ...
        HStack(spacing: 0) {
            List(pages, id: \.self, selection: $app.settingsPage) { page in
                Button { app.settingsPage = page } label: {
                    Text(page).font(.system(size: 13)).frame(maxWidth: .infinity, alignment: .leading).padding(.vertical, 4).contentShape(Rectangle())
            ...
            VStack(alignment: .leading, spacing: 0) {
                Text(app.settingsPage).font(.system(size: 21, weight: .semibold)).padding(24)
                ScrollView {
                    VStack(alignment: .leading, spacing: 18) {
                        page
                        if !status.isEmpty { Text(status).font(.system(size: 12)).foregroundStyle(app.pal.ink2) }
                        if let error = app.settingsError { Text(error).foregroundStyle(app.pal.ink2) }
                    }.padding(.horizontal, 24).padding(.bottom, 24).frame(maxWidth: .infinity, alignment: .leading)
                }
            }.frame(maxWidth: .infinity, maxHeight: .infinity, alignment: .topLeading)
        }.frame(width: 820, height: 620).font(.system(size: 13))
    ...
    @ViewBuilder private var page: some View {
        ...
        case "General":
            ...
            Text("Downloads ask where to save each file, starting in your chosen folder. Pins and playing tabs are protected from automatic archiving.").foregroundStyle(app.pal.ink3)
        ...
        case "Appearance":
            ...
            Text("Page font changes article text, not browser chrome or code blocks. Choose Website default to restore site typography. Browser controls use the system font. Reduce Transparency and Reduce Motion follow macOS Accessibility settings.").foregroundStyle(app.pal.ink3)
        ...
        case "Sites":
            if let error = app.sites.error { Text(error) }
            Text("Per-site zoom, permissions and blocking are available from the lock in the address toolbar. Settings are saved in sites.json.").foregroundStyle(app.pal.ink3)
            Text("Capture exclusions").font(.headline)
            Text("Excluded sites remain browsable, but Graphene does not capture their history or text.").foregroundStyle(app.pal.ink3)
            HStack {
        ...
        case "Privacy":
            ...
            Text("Bundled EasyList-derived network rules. Site controls can override this setting. Reload pages after changing blocking; WebKit does not expose a blocked-request count.").foregroundStyle(app.pal.ink3)
            ...
            Text("Spaces assigned to the same profile share cookies. Private windows use an ephemeral store and do not save browsing knowledge. Cookie behavior follows WebKit’s built-in tracking prevention; Graphene does not send a Do Not Track header.").foregroundStyle(app.pal.ink3)
            Divider()
            Text("Clear website data").font(.headline)
        ...
        case "Search":
            ...
            Text(app.settings.customSearchURL.map { "Active: \($0)" } ?? "Active: \(app.searchEngine.rawValue)").foregroundStyle(app.pal.ink3)
            Toggle("Search suggestions", isOn: $app.searchSuggestions)
            Text("Suggestions send search text to your selected engine. URLs and tab mentions stay local.").foregroundStyle(app.pal.ink3)
        case "AI":
            AISettingsView(root: app.dataDirectory)
        case "Shortcuts": ShortcutSettingsView()
        case "Mail":
            Label(mail.isConnected ? "Connected · read-only" : "Disconnected · read-only", systemImage: "envelope").font(.headline)
            Text("Gmail is read-only. Graphene cannot send, archive or delete mail.")
            if mail.isBusy { ProgressView("Connecting…") }
            if let error = mail.errorText { Text(error).foregroundStyle(app.pal.ink2) }
            ...
            if !mail.isConfigured { Text("Gmail OAuth is not configured in this build. You can still open Gmail on the web.").foregroundStyle(app.pal.ink3) }
            Button("Open Gmail on the web") { app.openTab(url: URL(string: "https://mail.google.com")!, parent: nil, activate: true) }
        default:
            Text("Data folder").font(.headline)
            ...
            Text("Debug driver: \(ProcessInfo.processInfo.environment["GRAPHENE_DEBUG"] == "1" ? "enabled" : "disabled"). Launch with GRAPHENE_DEBUG=1 and write commands to cmd.txt inside the isolated data folder. It is never enabled by a webpage.").foregroundStyle(app.pal.ink3)
        }
    }
...
struct RoutingSettingsView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 16) {
            ...
            Divider()
            Text("Routing · Air Traffic Control").font(.headline)
            Text("First matching rule wins. Match the whole URL with * (any text) and ? (one character). Routed links switch to the destination space.").foregroundStyle(app.pal.ink3)
            ...
            if let url = URL(string: testURL), !testURL.isEmpty {
                ...
                Text(rule.flatMap { rule in app.spaces.first { $0.id == rule.spaceID }?.name }.map { "Routes to \($0)" } ?? "No rule matches").foregroundStyle(app.pal.ink2)
            }
...
struct ShortcutSettingsView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            Text("macOS may own Control-number shortcuts. Websites can also intercept keys. Standard system editing shortcuts stay reserved.").foregroundStyle(app.pal.ink3)
            ForEach(["File", "Find", "View & Settings", "Spaces", "Tabs & History", "Knowledge"], id: \.self) { group in
                Text(group).font(.system(size: 13, weight: .semibold)).padding(.top, 12)
            ...
            VStack(alignment: .leading, spacing: 16) {
                Text("Change shortcut").font(.headline)
                ...
                Text(error).foregroundStyle(app.pal.ink2)

...
struct OnboardingView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 24) {
            HStack {
                Text("Welcome to Graphene").font(.system(size: 23, weight: .semibold))
                Spacer(); Text("\(step + 1) of 3").foregroundStyle(app.pal.ink3)
            }
            if step == 0 {
                Text("Choose how your tabs feel.").font(.system(size: 15))
                HStack(spacing: 18) {
                        ...
                        Button { app.layout = layout } label: {
                            VStack(spacing: 12) {
                                ZStack(alignment: layout == .sidebar ? .leading : .top) {
                                    RoundedRectangle(cornerRadius: 10).fill(app.pal.hover)
                                    RoundedRectangle(cornerRadius: 8).fill(app.pal.elev).padding(layout == .sidebar ? .leading : .top, 38).padding(8)
                                    VStack(spacing: 5) { ForEach(0..<3) { _ in Capsule().fill(app.pal.ink3).frame(width: layout == .sidebar ? 18 : 40, height: 3) } }
                                        .padding(14)
                                }.frame(width: 210, height: 140)
                                    .overlay(RoundedRectangle(cornerRadius: 10).strokeBorder(app.layout == layout ? app.pal.accentText : app.pal.hairline, lineWidth: 2))
                                Text(layout.title).font(.system(size: 13, weight: .medium))
                            }
                        }.buttonStyle(.plain)
                ...
                Text("Give your first space a color.").font(.system(size: 15))
                ...
                RoundedRectangle(cornerRadius: 12).fill(app.pal.sidebarBg).frame(height: 140)
                    .overlay(Text(app.activeSpace.name).font(.system(size: 20, weight: .medium)))
            } else {
                Text("Your pages, notes and Boards stay on this Mac.").font(.system(size: 15))
